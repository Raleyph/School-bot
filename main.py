...
def add_message(user_id: int, user_name: str, user_surname: str):
    conn = get_connection()
    c = conn.cursor()
    c.execute('INSERT INTO user_message (user_id, user_name, user_surname) VALUES (?, ?, ?)', (user_id, user_name, user_surname))
    conn.commit()
...
def send_welcome(message):
    ...
    if __name__ == '__main__':
        init_db()
        add_message(user_id=us_id, user_name=us_name, user_surname=us_sname)
